// AURELIA Plan — Content Part 3: Design System + Animation Strategy
...
function chapter5() {
  return [
    ...
    caption("Table 5.1 — Complete AURELIA colour token specification"),

    h2("5.3 Typography System"),

    p("AURELIA uses two typeface families: Fraunces (a variable serif) for display and headings, and Inter (a variable sans-serif) for body, UI, and microcopy. Both are loaded via next/font/google with the variable axis enabled, which gives access to the full weight range without shipping multiple font files. The choice of Fraunces is deliberate: it is a contemporary serif with optical sizing (the letterforms adjust based on size, becoming more refined at display sizes), a soft warmth that complements the gold palette, and a personality that distinguishes AURELIA from generic sans-only e-commerce sites. Inter is chosen as the body face because it was designed for screen readability at small sizes, has exceptional legibility, and is neutral enough to let Fraunces carry the brand voice."),
    ...
    caption("Table 5.2 — AURELIA fluid type scale (Fraunces for display/h1/h2, Inter for h3 and below)"),
...
export default function RootLayout({ children }: { children: React.ReactNode }) {
  return (
    ...
    caption("Table 5.3 — AURELIA spacing scale"),

    h2("5.5 Layout & Grid System"),

    p("AURELIA uses a 12-column grid on desktop, collapsing to a single column on mobile. The grid is implemented via Tailwind's grid utilities (grid-cols-12, col-span-X) and is constrained by a maximum content width of 1440px (90rem), with a minimum content width of 320px (the smallest phone AURELIA officially supports). The horizontal padding on either side of the grid is fluid: clamp(1rem, 5vw, 4rem), giving comfortable reading on small phones and generous air on large monitors. The grid is not used for every layout; for editorial pages (lookbook, journal) the layout breaks the grid intentionally for visual rhythm, but always returns to it for product listings and structured content."),
...
function chapter6() {
  return [
    ...
    p("Motion in AURELIA is not decoration; it is communication. Every animation has a purpose: to draw attention, to provide feedback, to guide the eye, to smooth a transition, or to delight. Animations without a purpose are removed. This discipline is critical because animations have a cost — they consume main-thread time, they can cause layout shift, and they can make a site feel sluggish if mistimed. The AURELIA motion system is engineered to feel expensive (long, smooth, choreographed) on capable devices and to feel instant on low-powered devices, with no middle ground that feels neither polished nor fast."),
    ...
    p("AURELIA motion follows four principles. First, slow is luxurious: AURELIA animations are slower than typical web animations (entrance animations are 800ms to 1200ms, not 300ms to 500ms) because slow motion signals confidence and craft. Second, choreography over chaos: when multiple elements animate together, they animate in sequence with small staggers (50ms to 120ms apart), never simultaneously, which creates a sense of choreography. Third, ease over line: every animation uses a custom cubic-bezier ease (a slow-in, slow-out curve with a slight overshoot for organic motion) rather than linear or default ease curves. Fourth, respect the user: every animation respects the prefers-reduced-motion media query, falling back to instant state changes for users who have requested reduced motion."),

    callout(
      "CUSTOM EASE CURVE",
      "AURELIA's signature ease: cubic-bezier(0.22, 1, 0.36, 1). This is a refined ease-out with no overshoot, derived from the standard 'easeOutQuint' curve. It feels slower and more deliberate than the default ease-out, which gives AURELIA motion its luxurious quality."
    ),

    h2("6.2 GSAP Usage Patterns"),

    p("GSAP is used via the @gsap/react package's useGSAP hook, which is the SSR-safe drop-in for useLayoutEffect and includes automatic cleanup via gsap.context(). Every GSAP animation in AURELIA lives inside a useGSAP hook, and the hook's scope is restricted to a ref to prevent selector collisions across components."),
