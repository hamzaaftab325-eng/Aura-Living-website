// AURELIA Plan — Content Part 2: Technical Architecture + Folder Structure
...
function chapter3() {
  return [
    ...
    caption("Table 3.1 — AURELIA frontend dependency manifest"),

    h2("3.2 Next.js 16 App Router Architecture"),

    p("The App Router is the default and only recommended router in Next.js 16. AURELIA uses it exclusively. The Pages Router is deprecated and will not be considered. The App Router's core architectural primitive is the layout: a React component that wraps a segment of the route tree and remains mounted across navigations within that segment. AURELIA uses a root layout (with the html and body tags, font loading, and global providers), a marketing layout (for landing, about, contact, blog pages), a shop layout (for product listing, product detail, cart, checkout — with persistent filter state and cart drawer), and an account layout (for authenticated pages)."),

    p("Each route segment can include optional special files: loading.tsx for Suspense fallbacks, error.tsx for error boundaries, not-found.tsx for 404 within that segment, and layout.tsx for the persistent wrapper. AURELIA uses loading.tsx on every dynamic route (product pages, product listing pages, search) to display a content-shaped skeleton that prevents layout shift and signals progress to the user. Error boundaries are route-segmented so that a failure in a single product card never takes down the entire listing."),
...
export const useCartStore = create<CartState>()(
  persist(
    ...
    { name: "aurelia-cart", storage: createJSONStorage(() => localStorage), skipHydration: true }
  )
);`),

    h2("3.6 Form Handling & Validation"),

    p("All forms in AURELIA — checkout, login, registration, newsletter, contact, address book — use React Hook Form for state management and Zod for validation. Zod schemas are defined once and shared between the client (for instant field-level feedback) and the future server (for authoritative validation). This eliminates the common anti-pattern of duplicating validation rules in two places and the bugs that follow."),
...
export const checkoutSchema = z.object({
  ...
  payment: z.object({
    ...
    caption("Table 3.2 — Complete route taxonomy for AURELIA"),
  ];
}
