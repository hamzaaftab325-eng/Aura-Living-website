// Aura Living Plan — Content Part 5: Components, SEO, Performance, Accessibility
...
function chapter8() {
  return [
    ...
    p("The Aura Living component inventory is organised in three tiers: UI primitives (the smallest reusable units, in features/ui/), composite components (combinations of primitives that solve a specific UX pattern, in features/<feature>/components/), and page sections (large compositions that make up the structure of a page, in features/home/components/ and features/layout/components/). Every component is a Server Component by default unless it requires interactivity, in which case it is explicitly marked with \"use client\". This chapter catalogues every component planned for the build."),

    h2("8.1 UI Primitives"),

    p("UI primitives are the building blocks. They are deliberately minimal, accepting className and standard HTML props for composition, and they enforce the design system by rejecting props that would override tokens (e.g., the Button component does not accept a backgroundColor prop). There are twelve primitives in the Aura Living design system."),
    ...
    caption("Table 8.1 — UI primitives in the Aura Living design system"),
...
export async function generateMetadata({ params }): Promise<Metadata> {
  ...
  return {
    ...
    caption("Table 9.1 — Schema.org markup implemented across Aura Living"),
...
function chapter10() {
  return [
    ...
    p("Every page in Aura Living ships only the JavaScript it needs. The Next.js App Router automatically code-splits at the route level, and Client Components are bundled separately from Server Components. Beyond the framework's defaults, Aura Living uses dynamic imports for heavy client-side dependencies that are not needed on initial load: the GSAP-heavy parallax and pinned-section components are dynamically imported on desktop only; the Stripe.js (future) library is loaded only on the checkout page; the Storybook-only code is excluded from production builds entirely."),
